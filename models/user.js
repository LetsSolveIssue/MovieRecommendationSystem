...
User.prototype.getDirectorMovies = function (callback) {
  var query = [
  ' MATCH (me:User)-[:RATED]->(m:Movie)<-[:DIRECTED]-(d:Director)-[:DIRECTED]->(m2:Movie) WHERE ID(me) = {userid} AND not (me)-[:RATED]->(m2)',
 'return distinct m2 AS movie, count(*) AS count',
    'Order BY count DESC',
    'LIMIT 10'
...
User.prototype.getActorMovies = function (callback) {
  var query = [
 ' MATCH (me:User)-[:RATED]->(m:Movie)<-[:ACTS_IN]-(d:Actor)-[:ACTS_IN]->(m2:Movie) WHERE ID(me) = {userid} AND not (me)-[:RATED]->(m2)',
 'return distinct m2 AS movie, count(*) AS count',
    'Order BY count DESC',
    'LIMIT 20'
