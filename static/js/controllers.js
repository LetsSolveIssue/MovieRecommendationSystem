...
moviesControllers.controller('RecommendationsCtrl', ['$scope', '$rootScope', '$location', '$http',
  function($scope, $rootScope, $location, $http, $cookieStore) {
    ...
    //Get director movies
    $scope.getDirectorMovies = function() {
      $http.get(rootURL+'/users/'+$rootScope.user.id+'/directorMovies').
      success(function(data, status, headers, config) {
        $scope.directorMovies = data;
      }).
      error(function(data, status, headers, config) {
        alert('Error loading director movies.');
      });
    }
    $scope.getDirectorMovies();

    //Get Actor movies
    $scope.getActorMovies = function() {
      $http.get(rootURL+'/users/'+$rootScope.user.id+'/actorMovies').
      success(function(data, status, headers, config) {
        $scope.actorMovies = data;
      }).
      error(function(data, status, headers, config) {
        alert('Error loading actor movies.');
      });
    }
    $scope.getActorMovies();
  }]);
